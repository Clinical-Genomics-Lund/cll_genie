...
class VQuest():
    ...
    def __init__(self, config: dict, output_dir: str, sample_id: str, run_type: str, submission_id: str):
        ...
        if self.run_type == "full":
            self.output_dir = Path(f"{output_dir}/{sample_id}/{submission_id}/vquest")
            self.vquest_results_file = f"{self.output_dir}/{self.sample_id}.zip"
            self.output_dir.mkdir(parents=True, exist_ok=True)
            

        elif self.run_type == "detailed":
            self.payload = self.subtypes_messages_payload()
            self.output_dir = Path(f"{output_dir}/{sample_id}/{submission_id}/vquest/detailed")

            self.vquest_results_file = f"{self.output_dir}/{self.sample_id}.txt"
            self.output_dir.mkdir(parents=True, exist_ok=True)
    ...
    def run_vquest(self) -> dict:
        ...
        #TODO: 
        sequences should be batched into sets of 50 (the most allowed by V-QUEST) and
    ...
    def save_zip_content(self, zip_data) -> None:
        ...
        with ZipFile(stream, 'r') as zip_file:
            for member_name in zip_file.namelist():
                ...
                output_file = Path(f"{self.output_dir}/{member_name}")
                with output_file.open('wb') as f:
                    f.write(member_content)

        with ZipFile(self.vquest_results_file, mode='w') as zip_file:
            for member_name in zip_file.namelist():
                zip_file.write(f"{self.vquest_results_file}/{member_name}", arcname=member_name)
    ...
    def process_zip_results_for_report(self) -> dict:
        ...
        parameter_dict = {}
        with open(f"{self.output_dir}/11_Parameters.txt", 'r') as f:
            for line in f:
        ...
        summary_raw_df = pd.read_csv(f"{self.output_dir}/1_Summary.txt", sep='\t', header=0)
        summary_raw_df = summary_raw_df.loc[:, ~summary_raw_df.columns.str.contains('^Unnamed')]
        summary_raw_df.fillna('',inplace=True)
        summary_raw_dict = summary_raw_df.groupby('Sequence ID').apply(lambda x: x.set_index('Sequence ID').to_dict('records')[0]).to_dict()
        VQuest.replace_empty_with_none(summary_raw_dict)

        # Processing Junction results
        junction_raw_df = pd.read_csv(f"{self.output_dir}/6_Junction.txt", sep='\t', header=0)
        junction_raw_df = junction_raw_df.loc[:, ~junction_raw_df.columns.str.contains('^Unnamed')]
        junction_raw_df.fillna('',inplace=True)
        junction_raw_dict = junction_raw_df.groupby('Sequence ID').apply(lambda x: x.set_index('Sequence ID').to_dict('records')[0]).to_dict()
        VQuest.replace_empty_with_none(junction_raw_dict)

        merged_dict_raw = self.create_dict_for_mongo(summary_raw_dict, junction_raw_dict, parameter_dict)
    ...
    def create_dict_for_mongo(self, s_dict, j_dict, p_dict):
        ...
        for seq_id in seq_ids:
            results_dict[self.sample_id][seq_id] = {'summary': s_dict[seq_id], 'junction': j_dict[seq_id]}
        ...
        for seq in text_content[1:]:
            ...
            for element in seq_elements[1:]:
                if 'Result summary' in element:
                    ...
                    indel_message = re.sub(f"Try 'Search for insertions and deletions'.*\n*", '', element)
    ...
    def process_text_results_for_report(self) -> dict:
        ...
        for seq_id, seq_value in text_dict.items():
            ...
            if 'CLL subset #' in seq_value['result']:
                ...
                processed_text_dict[seq_id]['CLL Subset Summary'] = seq_value['result'][start_index:]
            else:
                processed_text_dict[seq_id]['CLL Subset Summary'] = 'Inga delmängder har tilldelats' #'No Subsets have been assigned'

            if 'insertions' or 'deletions' in seq_value['message']:
                processed_text_dict[seq_id]['Indels if Any'] = seq_value['message'].split(':')[0]
    ...
    def process_config(config_dict):
        ...
        for key, value in vquest_payload.items():
            ...
            elif value.isdigit() or (value.startswith('-') and value[1:].isdigit()): #for negative numbers as well
